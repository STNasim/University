...
int main(int argc, char *argv[]) {
	...
	fstream bi; //Archivos de escritura
	...
	if(!op.is_open()){ //Se verifica que el archivo a leer haya sido abierto
	...
	bi.open("Actividad binario.bin",ios::out|ios::binary); //Se abre el archivo fisico binario
	
	for(int h=0;h<TATA;h++){
		bi.write((char*)&pc[h],sizeof(pc)); //Se guardan los resultados en el archivo
	}
	bi.write((char*)&TATA,sizeof(TATA)); 
	...
	for(int g=0;g<y;g++){	//Se guardan los datos en el archivo
		bi.write((char*)&res[g],sizeof(res));
	}
	...
	bi.write((char*)&G,sizeof(G));
	bi.write((char*)&A,sizeof(A));
	bi.write((char*)&T,sizeof(T));        	//Se guardan los datos en el archivo
	bi.write((char*)&ProC,sizeof(ProC));
	bi.write((char*)&ProG,sizeof(ProG));
	...
	bi.close (); //Se cierra el archivo bi
